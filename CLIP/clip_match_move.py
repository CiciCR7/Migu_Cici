...
# 参数配置
batch_size = 64  # 分批提取图片特征
similarity_threshold = 0.85  # 设定相似度阈值
...
for i in range(len(image_names)):
    ...
    # 将第一张图片复制到新文件夹并保留在源文件夹==========实际操作中也可以不复制到新文件夹，节省空间
    shutil.copy(image_paths[similar_images[0]], cluster_folder)

    # 将其余相似图片移动到新文件夹
    for idx in similar_images[1:]:
        if idx not in image_to_folder:  # 只处理尚未处理的图片
            shutil.move(image_paths[idx], cluster_folder)
            image_to_folder[idx] = cluster_folder
    
    folder_index += 1

print("Finished categorizing and moving images.")
